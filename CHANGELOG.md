...
- **기본 7단계** 표시 (`INITIAL_STEPS = 7`) / 전체 계산 상한 20단계 (`ABSOLUTE_MAX = 20`)
- **카드 클릭 시 다음 단계 자동 펼침** — 마지막 단계의 카드를 클릭하면 경로 선택과 동시에 다음 단계 확장
- 마지막 단계 헤더에 `›` 아이콘으로 더 보기 가능 여부 표시 (클릭 불가, 시각 힌트만)
- 확장 상태에서 **"접기"** 버튼 표시 → 클릭 시 7단계로 복귀
- **가로 스크롤**: 7단계 이하 `overflow: hidden`(스크롤 없음) / 8단계 이상 `overflow: auto`(스크롤 표시)
- 날짜 범위·표시 모드 변경 시 자동으로 7단계로 리셋
