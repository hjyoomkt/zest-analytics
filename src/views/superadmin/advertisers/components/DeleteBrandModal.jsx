...
export default function DeleteBrandModal({ isOpen, onClose, brand, onConfirm, isLoading }) {
  const [confirmText, setConfirmText] = useState("");
  const textColor = useColorModeValue("secondaryGray.900", "white");
  const expectedText = `${brand?.name} 브랜드 삭제에 동의합니다`;
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} size="lg" isCentered>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>브랜드 삭제 확인</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <VStack spacing="20px" align="stretch">
            ...
            <VStack align="flex-start" pl="20px" spacing="8px">
              <Text fontSize="sm" color={textColor}>
                • 브랜드 정보: <strong>{brand?.name}</strong>
              </Text>
              <Text fontSize="sm" color={textColor}>
                • 소속된 모든 사용자 계정
              </Text>
              <Text fontSize="sm" color={textColor}>
                • 광고 성과 데이터 (ad_performance)
              </Text>
              <Text fontSize="sm" color={textColor}>
                • API 토큰 및 연동 정보
              </Text>
