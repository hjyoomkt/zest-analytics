...
└── routes.js            라우트 정의 (Dashboard, NFT, Tables, Profile, ZestAnalytics, Auth 전체)
```
...
```
src/components/
├── HelmetProvider.js                페이지별 SEO 메타태그 관리 (PageHelmet)  ← 신규
...
├── navbar/
│   ├── NavbarAdmin.js               어드민 상단 내비게이션
│   ├── NavbarAuth.js                인증 페이지 내비게이션
│   ├── NavbarLinksAdmin.js          어드민 내비게이션 링크/드롭다운
...
```
src/contexts/
├── AuthContext.js       인증 상태 전역 관리 (useAuth 훅 제공)  ← 신규
├── DateRangeContext.js  날짜 범위 전역 관리 (useDateRange 훅 제공)  ← 신규
...
```
src/config/
└── supabase.js          Supabase 클라이언트 초기화  ← 신규
                         (환경변수 미설정 시 경고만 출력, 앱 실행 유지)
```
...
├── admin/
│   ├── default/                     메인 대시보드 페이지
│   │   ├── index.jsx                대시보드 페이지 컴포넌트
│   │   ├── components/
│   │   │   ├── CheckTable.js        체크박스 데이터 테이블
│   │   │   ├── ComplexTable.js      복합 데이터 테이블
│   │   │   ├── DailyTraffic.js      일일 트래픽 차트
│   │   │   ├── PieCard.js           파이 차트 카드
│   │   │   ├── Tasks.js             태스크 목록
│   │   │   ├── TotalSpent.js        지출 요약 차트
│   │   │   ├── UserActivity.js      유저 활동 타임라인
│   │   │   └── WeeklyRevenue.js     주간 수익 차트
│   │   └── variables/
│   │       ├── columnsData.js       테이블 컬럼 정의
│   │       ├── tableDataCheck.json  체크박스 테이블 샘플 데이터
│   │       └── tableDataComplex.json 복합 테이블 샘플 데이터
│   │
│   ├── marketplace/                 NFT 마켓플레이스 페이지
│   │   ├── index.jsx
│   │   ├── components/
│   │   │   ├── Banner.js            마켓플레이스 헤더 배너
│   │   │   ├── HistoryItem.js       구매 이력 아이템
│   │   │   └── TableTopCreators.js  TOP 크리에이터 테이블
│   │   └── variables/
...
│   │   ├── index.jsx
│   │   └── components/
│   │       ├── Banner.js / General.js / Information.js
│   │       ├── Notifications.js / Project.js / Projects.js
│   │       ├── Storage.js / Upload.js / Dropzone.js
│   │
...
│   │   └── variables/
│   │       ├── columnsData.js
│   │       └── tableData*.json (4개 샘플 데이터)
│   │
│   └── zestAnalytics/               Zest Analytics 전환 추적 분석 페이지  ← 신규
...
```
src/utils/
└── dateUtils.js         KST(UTC+9) 기준 날짜 계산 유틸 (getKSTNow/Yesterday/DaysAgo/Today, formatDateToYYYYMMDD)  ← 신규
```
...
└── img/
    ...
    ├── auth/auth.png, banner.png, lemon.jpg  ← lemon.jpg 신규 추가
...
## 향후 추가 예정 기능 (Google Analytics / 히트맵)
...
- **데이터 연동 로직**: `src/variables/` 또는 별도 `src/services/` 생성
- **API 환경 변수**: `.env`
- **사이드바 메뉴 추가**: `src/components/sidebar/components/Links.js`
...
- `.env` — Supabase URL/ANON_KEY를 growth-dashboard와 동일 값으로 설정
